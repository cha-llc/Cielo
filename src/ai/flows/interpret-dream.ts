...
const InterpretDreamInputSchema = z.object({
  dreamDescription: z.string().describe('A description of the dream to be interpreted.'),
});
export type InterpretDreamInput = z.infer<typeof InterpretDreamInputSchema>;

const InterpretDreamOutputSchema = z.object({
  analysis: z.string().describe('A detailed interpretation of the dream, including its themes and potential meanings.'),
  sentiment: z.string().describe('The overall sentiment or emotional tone of the dream (e.g., "Joyful", "Anxious", "Confusing").'),
  symbols: z.array(z.object({
    symbol: z.string().describe('A key symbol identified in the dream.'),
    meaning: z.string().describe('The potential meaning or interpretation of that symbol.'),
  })).describe('An array of key symbols found in the dream and their meanings.'),
});
